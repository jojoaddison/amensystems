...
/**
 * Integration tests for the {@link DigitalAssetResource} REST controller.
 */
@SpringBootTest(classes = AmensystemApp.class)
@AutoConfigureMockMvc
@WithMockUser
public class DigitalAssetResourceIT {
    ...
    private static final byte[] UPDATED_RESOURCE = TestUtil.createByteArray(1, "1");
     ...
     * if they test an entity which requires the current entity.
     */
    public static DigitalAsset createUpdatedEntity() {
        DigitalAsset digitalAsset = new DigitalAsset()
            .name(UPDATED_NAME)
    ...
    public void updateDigitalAsset() throws Exception {
        ...
        DigitalAsset updatedDigitalAsset = digitalAssetRepository.findById(digitalAsset.getId()).get();
        updatedDigitalAsset
            .name(UPDATED_NAME)
            .description(UPDATED_DESCRIPTION)
            .url(UPDATED_URL)
